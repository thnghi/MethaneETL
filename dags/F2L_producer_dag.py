...
def task_scan_file_and_create_run(upload_type):
    try:
        ...
        print(f"list_file_user: {list_file_user}")
        if len(list_file_user)>0: 
            master_worker = MasterRunWorker(pg_hook_landing,directory_path,upload_type)  
            ...
            for file_user in list_file_user:
                ...
                print(f"file_user: {file_user}")

                run_worker = UserRunWorker(pg_hook_landing,directory_path_user,src_user,master_worker.import_master_key,upload_type)  
...
with DAG(
    ...
    scan_user_upload = PythonOperator(
        task_id='task_scan_user_upload',
        python_callable=task_scan_file_and_create_run,
        op_kwargs={
    ...
    scan_system_upload = PythonOperator(
        task_id='task_scan_system_upload',
        python_callable=task_scan_file_and_create_run,
        op_kwargs={
            'upload_type': 'SystemUpload' 
        },
        provide_context=True,
    )
            

    [scan_user_upload,scan_system_upload]
